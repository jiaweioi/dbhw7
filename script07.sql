...
-- ANSWER2
CREATE TABLE TEMP1
AS SELECT NNumber,count(*) AS CNT 
FROM DOG 
GROUP BY NNumber;

CREATE TABLE ANSWER2
AS SELECT DISTINCT S.NNumber AS NNumber
FROM STUDENT S,TEMP1 T
WHERE Deposit >= 100 AND S.NNumber = T.NNumber AND T.CNT >= 3
...
WHERE D1 != '2' AND D1.NNumber = D2.NNumber AND D2.NNumber = D3.NNumber
